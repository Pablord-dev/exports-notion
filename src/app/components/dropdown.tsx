  ...
  return (
    <div className="relative" ref={ref}>
      <button type="button" disabled={disabled} aria-label={ariaLabel} aria-haspopup="listbox" aria-expanded={open}
              onClick={() => setOpen((v) => !v)}
              className="flex items-center gap-1.5 rounded-full border border-border bg-surface px-3 py-1 text-xs text-fg transition hover:border-blue disabled:cursor-not-allowed disabled:opacity-60">
        <span className="max-w-[11rem] truncate">{current}</span>
        <svg className={`h-3 w-3 shrink-0 text-muted transition-transform ${open ? "rotate-180" : ""}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 9l6 6 6-6" /></svg>
      </button>
      {open && (
        <ul role="listbox" aria-label={ariaLabel}
            className={`absolute left-0 z-40 max-h-60 min-w-full overflow-auto rounded-xl border border-border bg-surface p-1 shadow-xl ${openUp ? "bottom-full mb-1" : "top-full mt-1"}`}>
          {options.map((o) => (
            <li key={o.value} role="option" aria-selected={o.value === value}
                onClick={() => { onChange(o.value); setOpen(false); }}
                className={`cursor-pointer whitespace-nowrap rounded-lg px-3 py-1.5 text-xs transition ${
                  o.value === value ? "bg-blue text-white" : "text-fg hover:bg-dark-blue"
                }`}>
              {o.label}
            </li>
          ))}
