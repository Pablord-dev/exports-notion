...
3. Genera el hash bcrypt del password compartido:
   ```bash
   node -e "console.log(require('bcryptjs').hashSync(process.argv[1], 10))" "MI-PASSWORD"
   ```
   ⚠️ En `.env.local` el hash va con los `$` escapados (`\$2b\$10\$...`); en Vercel, literal.
...
3. Configura las env vars del `.env.example` en Project Settings. `DATABASE_URL` = **transaction
   pooler** (6543). `SYNC_BUDGET_MS=40000` (cap de 60s en Hobby). `APP_PASSWORD_HASH` **literal,
   sin escapar los `$`**. No cargues `LLM_OLLAMA_*` (localhost no existe en serverless).
...
- **Convención Next 16**: la protección de rutas vive en `src/proxy.ts` (ex-`middleware.ts`, renombrado 2026-07-06; el warning de deprecación desapareció). Además `src/instrumentation.ts` valida las 7 env vars obligatorias al arrancar el server (fail-fast): con vars faltantes el server no levanta y lista cuáles faltan.
