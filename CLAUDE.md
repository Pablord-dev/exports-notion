...
- `GET|PATCH|DELETE /api/admin/users` — administración de usuarios (protegido por el proxy). `GET` devuelve `{users: UserRow[], blocked: BlockedRow[]}` (las dos listas juntas: la pantalla las muestra a la vez); `PATCH` recibe `{email, role}`; **`DELETE ?email=` quita el acceso**: borra la fila de `users` **y** escribe en `blocked_users` — sin lo segundo la cookie de esa persona seguiría valiendo 7 días. `DELETE /api/admin/blocked?email=` restaura (no devuelve el rol: la fila se borró, vuelve como `viewer`). **`POST /api/admin/blocked` con `{emails: string[]}` bloquea por adelantado**, hayan entrado o no: es la única vía para cerrar la puerta *antes* del primer ingreso (el `DELETE` de arriba parte de una fila existente). Normaliza y deduplica, **valida todo antes de escribir nada** (un lote con un typo no debe quedar medio aplicado), tope de 50 por llamada, y a quien ya tenía fila se la borra para que no aparezca en las dos listas. Errores propios: **400 `bad_email`** con `invalid: [...]`, **400 `too_many`**. No filtra por `ALLOWED_EMAIL_DOMAINS`: la lista sólo deniega, así que anotar otro dominio no otorga nada. Exigen `admin`: **403 `forbidden`** si no. **409 `self`** al operar sobre uno mismo, **400 `bad_role`** con un rol inventado, **400 `bad_request`** sin email. ⚠️ El gate compartido (`src/lib/admin-actor.ts`) **no lleva `try/catch`**: acá un error de base tiene que cerrar la puerta.
...
- **`src/app/components/settings/`** — panel de configuración: modal grande centrado con secciones (Cuenta, Usuarios, Acerca de). **No reúsa `AppModal`**: ese es no-modal y está anclado a `top-16` por el onboarding, y acá se quiere lo contrario. La sección **Usuarios sólo existe para admins** (`canManageUsers`) y no se le veda al viewer con tooltip — no se le dibuja. Dentro de la tabla sí hay veto explicado: la **fila propia** lleva el rol y el «Quitar acceso» con `aria-disabled` + tooltip, porque el control existe en todas las demás filas. Quitar el acceso confirma **en la propia fila** (un Dialog dentro de otro Dialog trae problemas de foco) y agrega a esa persona al grupo **«Sin acceso»**, con su botón de restaurar. Ese grupo **se dibuja siempre, con lista o sin ella**: abajo lleva el campo que bloquea correos por adelantado (varios separados por coma, espacio o salto de línea), y esconderlo con la lista vacía dejaría sin ninguna vía el caso de cerrarle la puerta a alguien que todavía no entró. Los botones de restaurar llevan el correo en el `aria-label`: con dos filas, «Restaurar acceso» a secas no distingue a quién.
...
- **Quitar el acceso** (2026-08-11) — la sesión es una **cookie sellada de 7 días** y el server no lleva registro de cuáles existen: no hay nada que "cerrar". Por eso borrar la fila de `users` no le hacía nada a quien ya estaba adentro. La revocación es la tabla **`blocked_users`**, y se mira en tres lugares: el **callback de Google** (después del allowlist de dominio → `?error=blocked`), el **proxy** (401 + borra la cookie, en cada request protegido) y **`/api/auth/session`** (que está fuera del matcher, así que lo mira ella misma). ⚠️ Los tres son **fail-closed**, al revés de las lecturas de rol: si no se puede saber si alguien está bloqueado, no entra —tragarse el error dejaría pasar justo a quien se quiso sacar—. Consecuencia operativa: **con la migración sin aplicar, el proxy responde 503 a todo**. Aplicarla es requisito del despliegue, no un paso posterior.
  - Restaurar el acceso **no devuelve el rol**: la fila de `users` se borró al bloquear, así que vuelve como `viewer` y a un admin hay que promoverlo de nuevo.
  - El shell revisa `/api/auth/session` cada 60s (`SESSION_POLL_MS`) y llama a `onLogout` con `authenticated:false`: sin eso, quien perdió el acceso seguiría viendo una pantalla ya cargada hasta intentar algo. ⚠️ `onLogout` se lee **por ref**: las páginas lo pasan como lambda inline y con su identidad en las deps el intervalo se reiniciaba en cada render (misma trampa que `runAction` del tour). Cubierto por un E2E de **dos contextos** con reloj falso (`page.clock`) en `tests/e2e/settings.spec.ts`.
  - El E2E usa una **tercera identidad del stub-login** (`?role=descartable`): bloquear a `viewer` dejaría afuera a cualquier test paralelo con ese perfil, porque el memory-store es un singleton de proceso.
...
| `blocked_users` | Lista de bloqueo. `email` PK, `name` (copiado de `users` al bloquear), `blocked_at`, `blocked_by`. Sin TTL: dura hasta que un admin la levante. **Es condición de entrada** (login, proxy y `/api/auth/session` la consultan) al revés que `users`, que es registro. |
