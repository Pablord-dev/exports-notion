...
- `GET|PATCH|DELETE /api/admin/users` — administración de usuarios (protegido por el proxy). `GET` devuelve `{users: UserRow[], blocked: BlockedRow[]}` (las dos listas juntas: la pantalla las muestra a la vez); `PATCH` recibe `{email, role}`; **`DELETE ?email=` quita el acceso**: borra la fila de `users` **y** escribe en `blocked_users` — sin lo segundo la cookie de esa persona seguiría valiendo 7 días. `DELETE /api/admin/blocked?email=` restaura (no devuelve el rol: la fila se borró, vuelve como `viewer`). Exigen `admin`: **403 `forbidden`** si no. **409 `self`** al operar sobre uno mismo, **400 `bad_role`** con un rol inventado, **400 `bad_request`** sin email. ⚠️ El gate compartido (`src/lib/admin-actor.ts`) **no lleva `try/catch`**: acá un error de base tiene que cerrar la puerta.
